...
function getHtmlUrl(_path) {
    if (!_path) return '';
    var dnndocsBaseUrl = 'https://docs.sorted.com/';
...
function getTwitterShareHref(_path, title) {
    ...
    var text = encodeURIComponent(title) + 'docs.sorted.com';
...
function getFacebookShareHref(_path) {
    if (!_path) return '';
    var dnndocsBaseUrl = 'https://docs.sorted.com.com/';
    var url = encodeURIComponent(dnndocsBaseUrl + _path.replace('.md', '.html'));
...
function getFeedbackHref(docurl, title, uid) {
    if (!docurl) return '';
    var newIssueUrl = 'https://github.com/SortedGroup/doc-centre/issues/new';
...
var gitUrlPatternItems = {
    ...
    'vso': {
        ...
        // TODO: update to devops.azure.com
