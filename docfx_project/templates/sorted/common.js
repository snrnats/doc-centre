...
function getHtmlUrl(_path) {
    if (!_path) return '';
    var sortedBaseUrl = 'https://docs.sorted.com/';
    var htmlUrl = sortedBaseUrl + _path.replace('.md', '.html');
    return htmlUrl;
}
...
function normalizeGitUrlToHttps(repo) {
    ...
    var repoUrl = 'https://' + repo.substr(pos + 1).replace(/:[0-9]+/g, '').replace(/:/g, '/');
